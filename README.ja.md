...
| `/failed` | 失敗したタスクを表示 |
| `/status` | 現在の実行状態を確認、実行中のタスクをキャンセル |
| `/debug` | システム情報を表示 |
...
**Planモード**：複雑なタスクは自動的にClaudeをplanモード（`--permission-mode plan`オプション）で実行します。これにより、Claudeは実行前に実装アプローチを設計し、複雑な要件に対してより良い結果を得ることができます。

### ファイル添付

タスク作成時に要件や完了条件にファイルを添付できます：

1. 要件/完了条件の入力時に、まずファイル（画像、ドキュメントなど）を送信
2. 各添付ファイルの確認メッセージが表示されます
3. 最後に要件/完了条件をテキストで入力
4. 添付ファイルはタスクと一緒にClaudeに渡されます

**注意**: 添付ファイルはタスク完了/失敗/キャンセル時に自動的に削除されます。
...
- `/status`で実行中のすべてのタスクを表示（停止ボタンでキャンセル可能）
...
1. `/status`を送信して実行中のタスクを確認
2. 各実行中のタスクに「停止」ボタンが表示される
3. ボタンをクリックするとタスクが即座に終了
4. キャンセルされたタスクは失敗としてマークされる
