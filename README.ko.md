...
| `/completed` | 완료된 작업 보기 |
| `/failed` | 실패한 작업 보기 |
| `/status` | 현재 실행 상태 확인 및 실행 중인 작업 취소 |
| `/debug` | 시스템 정보 보기 |
| `/cancel` | 작업 생성 흐름 취소 |
...
**Plan 모드**: 복잡 작업은 자동으로 Claude를 plan 모드(`--permission-mode plan` 옵션)로 실행합니다. 이를 통해 Claude가 실행 전에 구현 계획을 설계하여 복잡한 요구사항에 대해 더 나은 결과를 얻을 수 있습니다.

### 파일 첨부

작업 생성 시 요구사항이나 완료 조건에 파일을 첨부할 수 있습니다:

1. 요구사항/완료 조건 입력 단계에서 먼저 파일(이미지, 문서 등)을 전송
2. 파일이 첨부되면 확인 메시지가 표시됨
3. 마지막에 요구사항/완료 조건을 텍스트로 입력
4. 첨부된 파일은 작업과 함께 Claude에게 전달됨

**참고**: 첨부 파일은 작업 완료/실패/취소 시 자동으로 삭제됩니다.
...
- `/status`로 모든 실행 중인 작업 확인 (중지 버튼으로 취소 가능)
- 높은 우선순위 작업이 우선 슬롯 획득

### 실행 중인 작업 취소

현재 실행 중인 작업을 취소할 수 있습니다:

1. `/status`를 전송하여 실행 중인 작업 확인
2. 각 실행 중인 작업에 "중지" 버튼이 표시됨
3. 버튼을 클릭하면 작업이 즉시 종료됨
4. 취소된 작업은 실패로 표시됨
...
### 작업이 진행 중 상태에서 멈춤
- 재시작 시 멈춘 작업은 자동으로 "ready" 상태로 리셋됨
