...
### 2026-01-14 적용 완료
1. **복잡 작업 Plan 모드 제거**
   - `--permission-mode plan` CLI 옵션 제거
   - 프롬프트로만 계획을 세우고 실행 완료하도록 변경
   - 이유: plan 모드가 너무 오래 걸리고 타임아웃 발생 빈번

2. **타임아웃 설정 기능** (config.taskTimeout)
   - config.json에서 작업 타임아웃 설정 가능 (기본값: 30분)
   - 초기 설정(`npx cc-telegram init`) 시 타임아웃 입력 추가
   - 기존 config에 taskTimeout이 없으면 마이그레이션 시 입력 요청
   - 범위: 5분 ~ 180분

3. **반복 실패 사유 상세화**
   - exitCode별 구체적인 실패 사유 표시
   - extractExitCodeReason() 함수 추가
   - 지원 코드: 1(일반오류), 2(잘못된사용), 126(권한없음), 127(명령없음), 130(인터럽트), 137(강제종료), 143(시그널종료)

4. **로케일 11개 언어 업데이트**
   - 타임아웃 관련 메시지 추가
   - exitCode 관련 실패 사유 메시지 추가
   - plan_instruction 프롬프트 수정

### 2026-01-13 적용 완료
1. **복잡 작업 Plan 모드 자동 적용** (2026-01-14에 제거됨)
